...
class DynamicUNet(nn.Module):
    ...
    def __init__(self, filters=5, input_channels=3, output_channels=2):
    ...
    def forward(self, x):
        ...
        # Encoding Part of Network.
        #   Block 1
        conv1 = F.tanh(self.conv1_1(x))
        conv1 = F.tanh(self.conv1_2(conv1))
        pool1 = self.maxpool1(conv1)
        #   Block 2
        conv2 = F.tanh(self.conv2_1(pool1))
        conv2 = F.tanh(self.conv2_2(conv2))
        pool2 = self.maxpool2(conv2)
        #   Block 3
        conv3 = F.tanh(self.conv3_1(pool2))
        conv3 = F.tanh(self.conv3_2(conv3))
        pool3 = self.maxpool3(conv3)
        #   Block 4
        conv4 = F.tanh(self.conv4_1(pool3))
        conv4 = F.tanh(self.conv4_2(conv4))
        pool4 = self.maxpool4(conv4)

        # Bottleneck Part of Network.
        conv5 = F.tanh(self.conv5_1(pool4))
        conv5 = F.tanh(self.conv5_2(conv5))

        # Decoding Part of Network.
        #   Block 4
        up6 = torch.cat((self.conv5_t(conv5), conv4), dim=1)
        conv6 = F.tanh(self.conv6_1(up6))
        conv6 = F.tanh(self.conv6_2(conv6))
        #   Block 3
        up7 = torch.cat((self.conv6_t(conv6), conv3), dim=1)
        conv7 = F.tanh(self.conv7_1(up7))
        conv7 = F.tanh(self.conv7_2(conv7))
        #   Block 2
        up8 = torch.cat((self.conv7_t(conv7), conv2), dim=1)
        conv8 = F.tanh(self.conv8_1(up8))
        conv8 = F.tanh(self.conv8_2(conv8))
        #   Block 1
        up9 = torch.cat((self.conv8_t(conv8), conv1), dim=1)
        conv9 = F.tanh(self.conv9_1(up9))
        conv9 = F.tanh(self.conv9_2(conv9))

        # Output Part of Network.
        output = F.softmax(self.conv10(conv9))
