...
def main():
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            optimizer_c.load_state_dict(checkpoint['optimizer_d_state_dict'])
            optimizer_m.load_state_dict(checkpoint['optimizer_k_state_dict'])
    ...
    # mR=torch.tensor([[[0.6442, 0.0928],
    #      [0.7166, 0.9541],
    #      [0.2668, 0.2831]]])
    # mR=mR.repeat(args.batch_size,1,1)
    ...
    for epoch in range(args.epoch_start, args.epochs):
        ...
        if epoch < pretraining_epoch:
            ...
            for ii, data in enumerate(data_loaders[_modes[0]]):
                ...
                out_MNet = mnet(y)
                kernel_l1_loss = F.l1_loss(out_MNet, mR)
            ...
            for ii, data in enumerate(data_loaders[phase]):
                ...
                loss.backward()
                total_norm_D = nn.utils.clip_grad_norm_(cnet.parameters(), args.clip_grad_D)
                total_norm_K = nn.utils.clip_grad_norm_(mnet.parameters(), args.clip_grad_K)
                ...
                loss_per_epoch['MSE'] += mse_loss.item() / num_iter_per_epoch[phase]
                ...
                mse_per_epoch[phase] += mse_loss / num_iter_per_epoch[phase]

                if (ii + 1) % args.print_freq == 0:
                    print(f"[Epoch:{epoch + 1:0>4d}/{args.epochs:0>4d}] {phase}: {ii + 1:0>5d}/{num_iter_per_epoch[phase]:0>5d}, "
                          f"NegLH={mse_loss.item():.4e}, KLvh_gauss={kl_vh_gauss.item():.4e}, KLve_gauss={kl_ve_gauss.item():.4e}, "
                          f"MSE={loss:.4e}, GradNormD:{args.clip_grad_D:.2e}/{total_norm_D:.2e}, "
                          f"GradNormK:{args.clip_grad_K:.2e}/{total_norm_K:.2e}, "
                          f"lrD={lr_C:.1e}, lrK={lr_M:.1e}")
                    print(f"m_h={out_MNet_mean[0, :, 0]}")
                    print(f"m_e={out_MNet_mean[0, :, 1]}")

                    writer.add_scalar('Iter Train Loss', loss.item(), step)
                    writer.add_scalar('Iter Train NegLogLikelihood', mse_loss.item(), step)
                    ...
                    writer.add_scalar('Iter Train MSE', mse_loss, step)
            ...
            print(f"m_h={out_MNet_mean[0, 0, 0, :]}")
            print(f"m_e={out_MNet_var[0, 0, 0, :]}")
            ...
            writer.add_scalar('Learning rate DNet', lr_C, epoch)
            writer.add_scalar('Learning rate KNet', lr_M, epoch)
        ...
        if (epoch + 1) % args.save_model_freq == 0 or epoch + 1 == args.epochs:
            ...
            torch.save({
                ...
                'optimizer_d_state_dict': optimizer_c.state_dict(),
                'optimizer_k_state_dict': optimizer_m.state_dict(),
            }, save_path_model)
            save_path_deblur_model_state = os.path.join(args.model_dir, 'model_state_d_' + str(epoch + 1))
            torch.save(cnet.state_dict(), save_path_deblur_model_state)
            save_path_kernel_model_state = os.path.join(args.model_dir, 'model_state_k_' + str(epoch + 1))
            torch.save(mnet.state_dict(), save_path_kernel_model_state)
        toc = time.time()
