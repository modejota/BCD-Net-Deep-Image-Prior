...
RGB_dir= '/work/work_ysw/Deblur_Code/test_data/center_0/annotated/'
patch = cv2.imread(RGB_dir+'patient_004_node_4_xini_87074_yini_13248.jpg')
...
C_GT= directDeconvolve(Y_GT.astype('float'),M_RUI)
get_range(C_GT)
...
for i in range(ns):
    S_od = np.dot(M_RUI[:, i].reshape(3, 1), C_GT[i, :].reshape(1, -1))
    S_rgb = od2rgb(S_od).astype('int')
    S_rgb = S_rgb.reshape(3, new_w, new_w).T
    plt.subplot(1, ns, i + 1)
    plt.imshow(S_rgb)
...
plt.figure(figsize=(15, 10))
Y_rec=np.dot(M_RUI,C_GT) # (3,2) x (2,MN)
get_range(Y_rec)
...
C_GT_norm= directDeconvolve(Y_GT_norm.astype('float'),M_RUI)
get_range(C_GT_norm)
...
example_dir='/work/work_ysw/Deblur_Code/test_result/center_0/annotated/'
...
# get_range(Y_rec)
# I_rec=od2rgb(Y_rec).astype('int')
# I_rec=I_rec.reshape(3,224,224).T
# get_range(I_rec)
# plt.title('I_rec')
# plt.imshow(I_rec)
...
np.linalg.norm(Y_GT-Y_rec.reshape(3,224,224).T)/Y_rec.size
...
plt.figure(figsize=(15,10))
n,bins,patches=plt.hist(C_GT[:2,:].T, color=['b','r'], bins=255)
...
get_range(I_rec)
plt.imshow(I_rec)
print(np.linalg.norm(Y_GT-Y_rec.reshape(3,224,224).T)/Y_rec.size)
