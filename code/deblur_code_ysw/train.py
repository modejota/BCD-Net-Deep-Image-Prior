...
def main():
    ...
    #     cnet = torch.nn.DataParallel(cnet,device_ids=[1,2])
    #     mnet = torch.nn.DataParallel(mnet, device_ids=[1,2])
    cnet.to(device)
    mnet.to(device)
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            cnet.load_state_dict(checkpoint['concentration_model_state_dict'], strict=True)
            mnet.load_state_dict(checkpoint['color_model_state_dict'], strict=True)
            step = args.step
    ...
    for epoch in range(args.epoch_start, args.epochs):
        ...
        valid_loss_per_epoch= {x: 0 for x in ['Valid_Loss', 'Valid_NegLH','Valid_kl_vh_gauss', 'Valid_kl_ve_gauss']}
        ...
        if epoch < pretraining_epoch:
            ...
            for ii, data in enumerate(data_loaders[_modes[0]]):
                ...
                pre_loss, pre_likelihood,pre_kl_vh_gauss, pre_kl_ve_gauss = loss_fn(out_CNet,out_MNet1, out_MNet2,y,
                                                         args.prior_sigma_h2,args.prior_sigma_e2, mR,args.alpha,args.pre_kl,args.pre_mse,pre_training=True
                                                         )
                pre_losses = pre_loss
            ...
            for ii, data in enumerate(data_loaders[phase]):
                ...
                loss,neg_lh,kl_vh_gauss, kl_ve_gauss = loss_fn(out_CNet,out_MNet1,out_MNet2,y,
                                                            args.prior_sigma_h2,args.prior_sigma_e2, mR,args.alpha,args.pre_kl,args.pre_mse,pre_training=False
                                                           )
                ...
                if (ii + 1) % args.print_freq == 0:
                    print(f"[Epoch:{epoch + 1:0>4d}/{args.epochs:0>4d}] {phase}: {ii + 1:0>5d}/{num_iter_per_epoch[phase]:0>5d}, "
                          f"loss={losses.item():.4e}, neglh={neg_lh.item():.4e},KLvh_gauss={kl_vh_gauss.item():.4e}, KLve_gauss={kl_ve_gauss.item():.4e}, "
                          f"MSE={mse:.4e}, GradNormC:{args.clip_grad_C:.2e}/{total_norm_C:.2e}, "
                          f"GradNormM:{args.clip_grad_M:.2e}/{total_norm_M:.2e}, "
                          f"lrC={lr_C:.1e}, lrM={lr_M:.1e}")
            ...
            print(f"{phase}: Epoch:{epoch + 1}, Loss={loss_per_epoch['Loss']:.4e}, NeGLH= {loss_per_epoch['NegLH']:.4e},"
            ...
            for phase in _modes[1:]:
                for ii, data in enumerate(data_loaders[phase]):
                    ...
                    with torch.no_grad():
                        ...
                        loss_valid, neg_lh_valid, kl_vh_gauss_valid, kl_ve_gauss_valid = loss_fn(out_CNet,out_MNet1,out_MNet2,y,
                                                            args.prior_sigma_h2,args.prior_sigma_e2, mR,args.alpha,args.pre_kl,args.pre_mse,pre_training=False
                                                                         )
                        loss = loss_valid
        ...
        if (epoch + 1) % args.save_model_freq == 0 or epoch + 1 == args.epochs:
            ...
            torch.save({
                ...
                'concentration_model_state_dict': cnet.state_dict(),
                'color_model_state_dict': mnet.state_dict(),
