...
SAVE_KEYS = ["cnet_name", "mnet_name", "patch_size", "pretraining_epochs", "n_samples", "theta_val", "sigmaRui_sq"]
...
MODEL_DIR_NAME = f"{args.cnet_name}_{args.mnet_name}_{args.pretraining_epochs}pe_{args.patch_size}ps_{args.theta_val}theta_{args.sigmaRui_sq}sigmaRui_{args.n_samples}nsamples"
...
cam_dataset_train = CamelyonDataset(args.camelyon_data_path, TRAIN_CENTERS, patch_size=args.patch_size, n_samples=int(0.3*args.n_samples))
cam_dataloader_train = torch.utils.data.DataLoader(cam_dataset_train, batch_size=1, shuffle=False, num_workers=args.num_workers)

cam_dataset_test = CamelyonDataset(args.camelyon_data_path, TEST_CENTERS, patch_size=args.patch_size, n_samples=int(0.2*args.n_samples))
cam_dataloader_test = torch.utils.data.DataLoader(cam_dataset_test, batch_size=1, shuffle=False, num_workers=args.num_workers)
...
metrics_camelyon_train = model.evaluate(cam_dataloader_train, prefix='train')
for k in metrics_camelyon_train.keys():
    results_dic_camelyon["camelyon_" + k] = metrics_camelyon_train[k]

metrics_camelyon_test = model.evaluate(cam_dataloader_test)
for k in metrics_camelyon_test.keys():
    results_dic_camelyon["camelyon_" + k] = metrics_camelyon_test[k]
