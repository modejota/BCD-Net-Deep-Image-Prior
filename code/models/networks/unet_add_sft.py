...
class SFT_Layer(nn.Module):
    ...
    def forward(self, feature_maps, M_maps):
        cat_input = torch.cat((feature_maps, M_maps), dim=1)
...
class Up(nn.Module):
    ...
    def forward(self, x, skip, M_code):    
        up = self.up(x)
...
class UNetAddSFT(nn.Module):
    ...
    def forward(self, x, out_M=None):
        ...
        out, skip1 = self.down1(out, M_code)
        out, skip2 = self.down2(out, M_code)
        out, skip3 = self.down3(out, M_code)
        out, skip4 = self.down4(out, M_code)


        out = self.mid_conv(out)

        out = self.up4(out, self.skip_conv4(skip4), M_code)
        out = self.up3(out, self.skip_conv3(skip3), M_code)
      
        out = self.up2(out, self.skip_conv2(skip2), M_code)
        out = self.up1(out, self.skip_conv1(skip1), M_code)
