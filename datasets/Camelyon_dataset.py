...
class CamelyonDataset(Dataset):
    def __init__(self, data_path, train_centers, patch_size, n_samples, augment=False, add_noise=False):
        super().__init__()
        ...
        self.image_list = self.scan_files(data_root_path,train_centers, n_samples)
        self.mR = torch.tensor([[0.6442, 0.0928],
             [0.7166, 0.9541],
             [0.2668, 0.2831]])
        # print('Reference matrix',self.mR,self.mR.size())

    def scan_files(self, data_root_path, train_centers, n_samples):
        ...
        OD_img_list = random.sample(train_patches, n_possible)
        # len(OD_img_list)
        return OD_img_list
...
class CamelyonValDataset(Dataset):
    # Patches are extracted at random
    def __init__(self, data_path, train_centers, patch_size, n_samples, augment=False, add_noise=False):
        super().__init__()
        data_root_path = data_path
        self.train_centers=train_centers
    ...
    def scan_files(self, data_root_path, train_centers, n_samples):
        ...
        # return image_list
        OD_img_list = []
        # n_samples = 10000  # Desired size for the dataset, you can change this
        #         camelyon_dir='/data/BasesDeDatos/Camelyon/Camelyon17/training/patches_224/'
        # train_centers = [0, 2, 4]  # This will take images from centers 0, 2 and 4
        tumor_patches_ids = []
        normal_patches_ids = []
